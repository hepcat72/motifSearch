...
my($outfile_suffix); #Not used, except expected to be here for subroutines
my $parent_files        = [];
my $input_files         = [];
...
my $GetOptHash =
  {'s|search-for=s'        => sub {push(@$motif_patterns, #REQUIRED unless <>
					[sglob($_[1])])}, #         is supplied
   '<>'                    => sub {push(@$motif_patterns, #REQUIRED unless -s
					[sglob($_[0])])}, #         is supplied
   'i|search-in=s'         => sub {push(@$parent_files,   #REQUIRED
					[sglob($_[1])])},
   'e|exact-matches-only!' => \$exact_only,               #OPTIONAL [Off]
   'c|case-sensitive!'     => \$case_sensitive,           #OPTIONAL [Off]
   'd|strand=s'            => \$strand,                   #OPTIONAL [both]
   'min_size=s'            => \$min_size,                 #OPTIONAL [10]
   'report-not-found!'     => \$report_not_found,         #OPTIONAL [Off]
...
unless(GetOptions(%$GetOptHash))
  {
    ...
    my @possibly_bad = grep {!(-e $_)} map {@$_} @$parent_files;
...
if($verbose && isStandardOutputToTerminal())
  {warning('You have enabled --verbose, but appear to possibly be ',
	   ...
	   'difficult to read.  You may want to either turn verbose off,  or ',
	   'redirect output to a file.')}

#Make sure there is input
if(scalar(@$motif_patterns) == 0 && isStandardInputFromTerminal())
  {
    error("-s (or input from redirection) is required.");
    quit(2);
  }

#Make sure there is input
if(scalar(@$parent_files) == 0)
  {
    error('No parent files detected (see -i).');
    usage(1);
    quit(1);
  }
...
my($input_file_sets,
   $outfile_stub_sets) = getFileSets($motif_patterns,
				     $parent_files);

##To allow users to submit raw sequence as a parameter, check the infile names
##to see if they exist and if they don't, number the output files sequentially
#my $outfile_count = 0;
#for(my $gi = 0;$gi < scalar(@$input_file_sets);$gi++)
#  {
#    for(my $i = 0;$i < scalar(@{$input_file_sets->[$gi]});$i++)
#      {
#	#If the file does not exist, assume they're submitting raw sequence
#	if(!(-e $input_file_sets->[$gi]->[$i]))
#	  {
#	    #And create a numeric file name
#	    $outfile_count++;
#	    $outfile_stub_sets->[$gi]->[$i] = $outfile_count;
#	  }
#      }
#  }
...
foreach my $set_num (0..$#$input_file_sets)
  {
    ...
    my $parent_file  = $input_file_sets->[$set_num]->[1];
    my $outfile_stub = $outfile_stub_sets->[$set_num]->[1];
...
#      {$motif_pattern = getPatternFromFile($input_file)}
    if(-e $input_file || $input_file eq '-')
      {
...
#    else
#      {$motif_pattern = $input_file}
    else
      {push(@target_records,[">$outfile_stub",$input_file])}
	...
	verbose("Looking for pattern: [$pattern] in [$parent_file].");
      }
...
#	unless($dry_run)
#	  {
#	    @target_records = getNextFastaRec(*INPUT);
#	    closeIn(*INPUT);
#	  }
#      }
#    else #Assume that the submitted string is the sequence itself
#      {push(@target_records,[">$outfile_stub",$input_file])}

#    push(@target_records,[">$outfile_stub",$motif_pattern]);
    ...
    #Assume the submitted string is the sequence itself if it looks like DNA
      ...
      {push(@source_records,[">$outfile_stub",$parent_file])}
    else
      {
	error("-i does not appear to be either a file or raw DNA sequence: ",
	      "[$parent_file].  Skipping this source file.");
	next;
      }

#    verbose("Looking for [$motif_pattern] in ",
#	    "[$parent_file].");
...
#    while(my $source_rec = ($file_input ? getNextFastaRec(*PARENT) : shift()))
      {
...
#		my($match_seq,$rc_match_seq);
		my $match_seq = $target_sequence;
		if(!$case_sensitive)
		  {$match_seq = "(?i:$match_seq)"}
#		if($exact_only)
#		  {$match_seq = $target_sequence}
#		else
#		  {$match_seq = getDNAPattern($target_sequence)}

#		#Make sure the sequence is DNA if not in exact_only mode
#		if(!$exact_only && !isDNA($target_sequence))
#		  {
#		    warning("Sequence(s) were encountered in subseq file ",
#			    "[$input_file] that appear to not be DNA when ",
#			    "not running in --exact-matches-only mode (e.g.: ",
#			    "'$target_defline').  Sequence treated as DNA ",
#			    "will not extract correct coordinates and you ",
#			    "will get lots of other errors.  Please check ",
#			    "your file to make sure it is DNA or turn on ",
#			    "--exact-matches-only.");
#		  }
...
#		my $rc_sequence  = '';
#		if(isDNA($target_sequence))
#		  {
#		    $rc_sequence = reverseComplement($target_sequence);
#		    if($exact_only)
#		      {$rc_match_seq = $rc_sequence}
#		    else
#		      {$rc_match_seq = getDNAPattern($rc_sequence)}
#		  }
		...
		verbose("$str  Done.  Sequences found: $seqs_found  Time ",
			'taken: [',scalar(markTime()),' Seconds].');
	      }
	    else
...
sub sglob
  {
    ...
    if(scalar(@arguments) > scalar(@partials) &&
       scalar(grep {-e $_} @arguments) == 0)
      {return($command_line_string)}

    debug({LEVEL => -2},"Returning split args: [(",join('),(',@arguments),
...
#Copied from template version 2.10
sub getFileSetsOLD
  {
    my($file_types_array,$outdir_array);
    my $outfile_stub = 'STDIN';
...
* WHAT IS THIS: Searches for perl regular expressions in sequence files.  This
                script outputs a tab-delimited file with coordinates.

* SEARCH-FOR FORMAT: Text file where each line is a separate perl regular
                     expression (i.e. 'pattern') to search for in all the
                     sequences of the SEARCH-IN files.  No tabs are allowed in
                     a pattern.  Output will indicate the pattern matched by
                     the number of the line on which the pattern was found.
                     Optionally, a pattern may be preceded by an ID followed by
                     a tab character.  An ID may not have any spaces or tabs in
                     it.  The pattern may be followed by a tab and comments
                     that will be ignored.  Commented lines (lines starting
                     with '#') and lines with nothing on them will be ignored.

* SEARCH-IN FORMAT: Fasta sequence file with a unique ID on the defline.

* OUTPUT FORMAT: Tab delimited file containing columns: file name/path where
                 the pattern was found, source/parent sequence ID, start
                 coordinate, stop coordinate, and strand (+/-).  Example:
...
sub usage
  {
    ...
    print("\n$script -i \"search_in_file(s)\" $options < pattern_file\n",
	  "$script -s \"pattern_file(s)\" -i \"search_in_file(s)\" $options\n",
	  "$script \"pattern_file(s)\" -i \"search_in_file(s)\" $options\n",
	  "$script -s \"pattern\" -i \"sequence\" $options\n\n");
    ...
    if($no_descriptions)
      ...
      {
        print << 'end_print';
     -s|--search-for      REQUIRED Perl regular expression to search for in the
                                   source sequences.
     -i|--search-in       REQUIRED Space-separated sequence file(s) or sequence
                                   strings to search in.  Expands glob
                                   characters ('*', '?', etc.).  When standard
                                   input detected, used as a file name stub
                                   (See -o).  See --help for file format and
                                   advanced usage.
     -e|--exact-matches-  OPTIONAL Search for exact matches only and treat
        only                       ambiguous DNA characters like any other
                                   character.  Use this option for non-DNA
                                   sequence searches.
     -c|--case-sensitive  OPTIONAL Default behavior is case-insensitive.
                                   Supply this flag to match regardless of
                                   case.
     --strand             OPTIONAL [both]{forward,reverse,both} If the sequence
                                   supplied to -i is DNA, search the indicated
                                   strand(s).  If any sequence has non-DNA
                                   characters, no attempt will be made to
                                   search the reverse complement.
     --min_size           OPTIONAL [2] Ignore sequences supplied by -s that are
                                   smaller than this size.
     --report-not-found   OPTIONAL Report empty records when a search-for
